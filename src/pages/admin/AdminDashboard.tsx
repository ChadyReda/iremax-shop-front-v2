    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {CARDS.map(({ label, value, sub, Icon, color }) => (
          <div key={label} className="bg-white border border-stone-100 p-5">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {/* Recent orders */}
        <div className="lg:col-span-2 bg-white border border-stone-100 p-5">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-semibold text-stone-900 text-sm uppercase tracking-widest">
              Recent Orders
            </h2>
            <Link
              to="/admin/orders"
          ...
          <div className="space-y-3">
            {recentOrders.map((o) => (
              <div
                key={o._id}
                className="flex items-center justify-between gap-4 py-2 border-b border-stone-50 last:border-0"
              >
                <div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-stone-100 p-5">
            <h2 className="font-semibold text-stone-900 text-sm uppercase tracking-widest mb-4">
              Orders by Status
            </h2>
          ...
          {lowStockProducts.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-semibold text-amber-800 text-sm uppercase tracking-widest">
                  Low Stock
                </h2>
              </div>
              <div className="space-y-2">
                {lowStockProducts.slice(0, 5).map((p) => (
                  <div key={p._id} className="flex justify-between items-center">
                    <p className="text-xs text-amber-800 truncate max-w-[160px]">
                      {p.name}
                    </p>
              ...
              <Link
                to="/admin/products"
                className="text-xs text-amber-600 hover:text-amber-800 font-medium mt-3 block"
              >
                Manage inventory →
              </Link>
            </div>
      ...
      {/* Top products */}
      <div className="bg-white border border-stone-100 p-5 mt-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-stone-900 text-sm uppercase tracking-widest">
            Top Products
          </h2>
                ...
                <tr key={p._id}>
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-stone-900 text-sm line-clamp-1">
                        {p.name}
                      </span>
                    </div>
                  </td>
                  <td className="py-3 pr-4 text-stone-600">{fmt(p.price)}</td>
                  <td className="py-3 pr-4 text-stone-600">{p.totalSold}</td>
